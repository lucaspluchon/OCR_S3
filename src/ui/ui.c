...
void Create_MessageBox(GtkWindow* window, char* message)
{
...
{
    gdk_window_set_cursor(gtk_widget_get_window(GTK_WIDGET(data->ui.window_main)), data->ui.watch_cursor);
    ...
    gdk_window_set_cursor(gtk_widget_get_window(GTK_WIDGET(data->ui.window_main)), NULL);
}

void on_image_choose(GtkFileChooserButton *widget, gpointer user_data)
{
    ocr_data* data = user_data;
    gtk_widget_set_visible(GTK_WIDGET(data->ui.progress_main), gtk_true());
    while (g_main_context_iteration(NULL, FALSE));
    ...
    gtk_widget_show(GTK_WIDGET(data->ui.window_image));
    gtk_widget_hide(GTK_WIDGET(data->ui.window_main));
    gtk_widget_set_visible(GTK_WIDGET(data->ui.progress_main), gtk_false());
...
void on_confirmAnalyse(GtkFileChooserButton *widget, gpointer user_data)
{
    ...
    {
        Create_MessageBox(data->ui.window_image,"Neural network is not detected !");
        return;
    }
    if (strcmp(data->file_path,"") != 0)
        g_free(data->file_path);
    ...
    gtk_widget_set_visible(GTK_WIDGET(data->ui.progress_neural), gtk_true());
    gdk_window_set_cursor(gtk_widget_get_window(GTK_WIDGET(data->ui.window_main)), data->ui.watch_cursor);
    while (g_main_context_iteration(NULL, FALSE));
    ...
    gtk_widget_show(GTK_WIDGET(data->ui.window_result));
    gtk_widget_hide(GTK_WIDGET(data->ui.window_image));

    gtk_widget_set_visible(GTK_WIDGET(data->ui.progress_neural), gtk_false());
    gdk_window_set_cursor(gtk_widget_get_window(GTK_WIDGET(data->ui.window_main)), NULL);

    if (data->text_array->nb_block == 0)
    {
        Create_MessageBox(data->ui.window_image,"No text found in this image.");
    }
...
{
    if (gtk_builder_add_from_file(builder, "ui_glade/ui_ocr.glade", &error) == 0)
    {
